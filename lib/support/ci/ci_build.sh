#! /bin/bash
# set -vx
# CI_BUILD_PROJECT_NBR=1 CI_BUILD_ID=2323 CI_BUILD_REF=be44f8299de7605f4bdc10019270afaaaedd109f CI_BUILD_REF_NAME=feature-61068370-edit-grad-education PWD=/home/gitlab_ci_runner/gitlab-ci-runner/tmp/builds/project-1 . ci_build.sh
# 
# 
# This is invoked by GitLab CI on each build run and meant to be project specific.
...
# ----------------------
# 1. Push your project to your remote repository to trigger a CI build. If all configured properly,
...
CI_HTTP_HOST="http://ci.colddata.com"
CI_BUILD_PROJECT_Name="Beotracker.com"

CI_BUILD_PROJECT_NBR=
MAIL_FROM="GitLab CI Runner <gitlab_ci_runner@int.colddata.com>"
MAIL_TO="$(git show  --pretty=tformat:%ae $CI_BUILD_REF | head -n 1)"
MAIL_CC="beotracker-developers@colddata.com"
...
build() {
  ...
  CI_BUILD_LOG+=$(echo -e "\n\n2. Starting DB setup...")
  CI_BUILD_LOG+=$(echo -e "\n"; RAILS_ENV=$RAILS_ENV bundle exec rake db:drop db:setup;)
  [ $? -ne 0 ] && CI_BUILD_EXIT_CODE=1 && return
